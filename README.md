...
- **Optional live weather (offline-safe):** set `OPENWEATHER_API_KEY` (and optionally `OPENWEATHER_CITY`, default `Delhi`) in `.env` to enrich the dashboard with current temperature/humidity/rainfall from OpenWeatherMap. Fetches are cached for 30 minutes and time out after 3 seconds; if the request fails for any reason (offline, revoked key, rate limit) the app silently falls back to the local sensor readings, so the farm keeps working with no internet at all. The dashboard footer shows which source the climate values came from.
